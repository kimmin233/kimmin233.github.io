...
이런 데이터들이면 x값(크기)보단 y값(무게)에만 영향을 받아서 오로지 y값(무게)만 고려 대상이 된다. <br>
...
계산하는 방법은 평균을 빼고 표준편차를 나누어 주는 것이다. <br>
넘파이에서 제공하는 함수를 이용하자. <br>
...
**mean()**힘수는 **평균을 계산**하고, **std()**함수는 **표준편차를 계산**한다. <br>
